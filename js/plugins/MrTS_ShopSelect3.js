...
(function() {
	...
	Game_Interpreter.prototype.pluginCommand = function(command, args) {
		...
		if (command.toLowerCase() === "shopselection3") {
	...
	Window_MapSelect_Message.prototype.refresh = function() {
		...
		this.drawText("钥匙商店", 0, 0, this.contentsWidth(), 'center');
		this.resetTextColor();
		this.drawText("有以下两种：", 0, 76, this.contentsWidth(), 'left');
	};
	...
	Window_MapSelect_List.prototype.drawItem = function(index) {
		...
		if (item)
		{
			if (index === 0 && $gameParty.gold() < 20){
				this.changeTextColor(this.textColor(8));
			}
			if (index === 1 && $gameParty.gold() < 80){
				this.changeTextColor(this.textColor(8));
			}
    ...
    Window_MapSelect_List.prototype.isCurrentItemEnabled = function(index) {
	    ...
	    // return enabled;
	    // return true;
	    if (this._index === 0 && $gameParty.gold() < 20){
			return false;
		}
		if (this._index === 1 && $gameParty.gold() < 80){
			return false;
		}
		return true;
	};
